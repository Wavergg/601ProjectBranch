            ...
            <table class="table table-hover mt-5 mr-5" id="candidateTable">
                ...
                <tbody>
                    <tr v-for="candidate in candidatesCopy" v-bind:class="{ 'font-italic text-danger': compareDate(candidate.ApplyDate) }" :key="candidate.CandidateID" :id="'row'+candidate.CandidateID">
                        ...
                        <th class="textInfoPos"><span class="textInfo text-center" style="left: 0px;width:190px;">See Applicant's Details</span><a v-on:click="getUrl(candidate.CandidateID)" role="button" class="text-primary"><i style="font-size:30px;" class="ml-1 icon ion-md-document mx-3"></i></a></th>
                        <th class="textInfoPos" ><span class="textInfo text-center" style="left: -45px;width:160px;">Download <br>Applicant's CV</span><a class="btn btn-outline-dark px-2" :href="'<?php echo base_Url(); ?>index.php/candidateMission/downloadCV/'+candidate.CandidateID+'/'+ candidate.JobCV">CV</a></th>
